...
class FamilyManagementScreen extends ConsumerWidget {
  ...
  Future<void> _inviteParent(
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('邀请已创建，对方登录后将自动加入家庭')));
    }
  }
  ...
  Future<void> _setParentPin(
    ...
    ref.read(parentModeProvider.notifier).state =
        verified.verified && verified.authorizedUntil != null;
    if (context.mounted) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('家长 PIN 已更新')));
    }
  }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Scaffold(
      ...
      body: ref.watch(activeParentFamilyProvider) == null
          ? const Center(child: Text('家长授权已到期，请返回首页重新验证 PIN'))
          : ref
                ...
                .when(
                  data: (families) => ListView(
                    ...
                    children: [
                      for (final family in families.where(
                        (f) => f.id == ref.watch(activeParentFamilyProvider),
                      )) ...[
                        ListTile(
                          ...
                          trailing: Wrap(
                            ...
                            children: [
                              ...
                              FilledButton.tonalIcon(
                                onPressed: () => _guard(
                                  context,
                                  () => _editChild(context, ref, family.id),
                                ),
                  ...
                  error: (error, stackTrace) =>
                      Center(child: Text('家庭数据读取失败：$error')),
                ),
    );
  }
}
